...
export class SigninController {
    login(email, password) {
        ...
        return userModel.findByEmailAndPassword(email, hashUtil.encryptPassword(password))
            .then((user) => {
            ...
            // .then((user) => {
            //     logger.info(` [login] user fetched successfully
				//     \n${user}`);
            //
            //     if (!user) {
            //         throw new Error(AUTH_ERROR);
            ...
            //     _user = user;
            //
            //     return companyModel.findByUserId(user._id);
            // })
            // .then((company) => {
            //     logger.info(` [login] user fetched successfully
				//     \n${company}`);
            //
            //     return Promise.resolve(getSigninResultEntity(_user, company));
            // })
            .catch((err) => {
    ...
    // _getVerificationCodeHash() {
    //     logger.info(` [_getVerificationCodeHash] attempt to generate verification code hash`);
    //
    //     const verificationCodeModel = ModelLocator.getInstance().getVerificationCodeModel();
    //
    //     let count = 5;
    ...
    //
    //         return verificationCodeModel.findByHash(hash)
